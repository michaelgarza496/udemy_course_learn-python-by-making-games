...
def load_image(file_path: str, convert_alpha: bool = True) -> Surface:
    ...
    return load(file_path).convert_alpha() if convert_alpha else load(file_path).convert()
...
def load_images_animation(files_path: str, convert_alpha: bool = True) -> dict:
    ...
    for index, content in enumerate(os.walk(files_path)):
        ...
        root = root.replace('\\', '/')
        key = root.split('/')[-1]
        images[key] = [load_image(f'{root}/{img}', convert_alpha=convert_alpha) for img in sorted(files)]
    return images
